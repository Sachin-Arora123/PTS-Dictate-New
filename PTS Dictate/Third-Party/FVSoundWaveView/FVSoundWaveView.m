...
- (void) render {
    ...
    _waveImageView.width = self.width;
...
- (UIImage*) renderWaveImageFromAudioAsset:(AVURLAsset *)songAsset {
    ...
    if (reader.status == AVAssetReaderStatusCompleted)
    {
        ...
        if (image.size.width < self.width) {
            image = [self imageWithImage:image scaledToSize:_waveImageView.frame.size];
        }
        
        [PTSHELPER hideLoadingIndicator:APPDELEGATE.navController.visibleViewController];
