...
class ExistingVC: BaseViewController {
    ...
    @IBOutlet weak var mediaProgressView: SCWaveformView!
...
//        mediaProgressView.doesAllowScroll = true
...
//        self.mediaProgressView.gradientEndColor = #colorLiteral(red: 0.6509803922, green: 0.8235294118, blue: 0.9529411765, alpha: 1)
...
//        self.mediaProgressView.stop()
        self.mediaProgressView.normalColor = #colorLiteral(red: 0.6509803922, green: 0.8235294118, blue: 0.9529411765, alpha: 1)
        self.mediaProgressView.progressColor = #colorLiteral(red: 0.2273887992, green: 0.2274999917, blue: 0.9748747945, alpha: 1)
        mediaProgressView.timeRange = CMTimeRangeMake(start: CMTime.zero, duration: CMTimeMakeWithSeconds(1, preferredTimescale: 1))
        // Set the precision, 1 being the maximum
        mediaProgressView.precision = 0.25 // We are going to render one line per four pixels

        // Set the lineWidth so we have some space between the lines
        mediaProgressView.lineWidthRatio = 1

        // Show stereo if available
        mediaProgressView.channelStartIndex = 0
        mediaProgressView.channelEndIndex = 1

        // Show only right channel
        mediaProgressView.channelStartIndex = 1
        mediaProgressView.channelEndIndex = 1

        // Add some padding between the channels
        mediaProgressView.channelsPadding = 10
    }
...
//            self.mediaProgressView.audioURL = completePathURL!
            self.mediaProgressView.asset = AVURLAsset(url: completePathURL!)
        }else{
...
////            let samplesToHighlight = Int(arc4random()) % self.mediaProgressView.totalSamples
//            let samplesToHighlight =  self.mediaProgressView.totalSamples / 2
//            self.mediaProgressView.highlightedSamples = 0 ..< samplesToHighlight
//        }
//        let totalSamples = self.mediaProgressView.totalSamples / fileDuration
//       UIView.animate(withDuration: TimeInterval(fileDuration), delay: 0, options: .curveLinear, animations: {
//                self.mediaProgressView.highlightedSamples = 0..<totalSamples
//            })
    }
}
